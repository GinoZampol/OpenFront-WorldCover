...
export default defineConfig(({ mode }) => {
  ...
  const requestedStaticBase = env.STATIC_BASE ?? "/";
  const staticBase = requestedStaticBase.endsWith("/")
    ? requestedStaticBase
    : `${requestedStaticBase}/`;
  ...
  const staticHostFallback = (): Plugin => ({
    ...
    closeBundle() {
      const outDir = path.join(__dirname, "static");
      // Cloudflare Pages reads _redirects. GitHub Pages serves 404.html for
      // client-side game routes such as /w1/game/<id> after a refresh.
