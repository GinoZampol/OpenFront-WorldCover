...
import { GameMapImpl, LandValueMode } from "../../src/core/game/GameMap";
import {
  GameUpdateType,
  GameUpdateViewData,
  PlayerUpdate,
...
export function stubConfig(overrides: Partial<Config> = {}): Config {
  ...
  const cfg = {
    ...
    gameConfig: () => ({ gameMap: GameMapType.World }),
...
export function stubTerrainMap(
  width = 10,
  height = 10,
  terrain: Uint8Array<ArrayBufferLike> = new Uint8Array(width * height),
  landValueMode: LandValueMode = "uniform",
): TerrainMapData {
  const numLandTiles = terrain.reduce(
    (count, tile) => count + (tile & 0x80 ? 1 : 0),
    0,
  );
