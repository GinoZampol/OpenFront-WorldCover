...
describe("GameView.update — packed channels", () => {
  it("packedPlayerUpdates tuples update tilesOwned/landValue/gold/troops", () => {
    const game = makeGameView();
    game.update(
      withPlayers(1, [
    ...
    // [smallID, tilesOwned, landValue, gold, troops]
    gu.packedPlayerUpdates = new Float64Array([1, 42, 1234, 999, 250]);
    game.update(gu);

    const alice = game.player("alice");
    expect(alice.numTilesOwned()).toBe(42);
    expect(alice.landValue()).toBe(1234);
  ...
  it("gold survives the float64 quad exactly, including > 2^32 values", () => {
    ...
    const gu = makeEmptyGu(2);
    gu.packedPlayerUpdates = new Float64Array([1, 0, 0, bigGold, 0]);
    game.update(gu);
...
describe("GameView.update — tile deltas", () => {
  it("tracks WorldCover value contributed by each terrain class per player", () => {
    const terrain = new Uint8Array([0x80, 0x81, 0x82, 0x83]);
    const config = stubConfig({
      gameConfig: () => ({ gameMap: GameMapType.WorldCover }) as any,
    });
    const game = makeGameView({
      width: 2,
      height: 2,
      terrain,
      landValueMode: "worldcover",
      config,
    });
    const first = withPlayers(1, [
      makePlayerUpdate({ id: "alice", smallID: 1 }),
      makePlayerUpdate({ id: "bob", smallID: 2 }),
    ]);
    first.packedTileUpdates = new Uint32Array([
      0,
      (0x80 << 16) | 1,
      1,
      (0x81 << 16) | 1,
    ...
    expect(
      Array.from(game.worldCoverLandValueBreakdown(game.player("alice"))),
    ).toEqual([150, 5, 0, 0, 0, 0, 0, 0]);
    expect(game.worldCoverLandValueBreakdown(game.player("bob"))[2]).toBe(0.2);

    const conquest = makeEmptyGu(2);
    conquest.packedTileUpdates = new Uint32Array([0, (0x80 << 16) | 2]);
    game.update(conquest);

    expect(game.worldCoverLandValueBreakdown(game.player("alice"))[0]).toBe(0);
    expect(game.worldCoverLandValueBreakdown(game.player("bob"))[0]).toBe(150);
  });
