...
describe("Warship", () => {
  ...
  test("Warship ignores a port missing from the water navigation mesh", () => {
    ...
    const warshipTile = game.ref(coastX + 1, 11);
    const warship = player1.buildUnit(UnitType.Warship, warshipTile, {
      patrolTile: warshipTile,
    });
    game.addExecution(new WarshipExecution(warship));

    const originalGetWaterComponent = game.getWaterComponent.bind(game);
    vi.spyOn(game, "getWaterComponent").mockImplementation((tile) =>
      tile === portTile ? null : originalGetWaterComponent(tile),
    );
    ...
    warship.modifyHealth(-(maxHealth - 1));
