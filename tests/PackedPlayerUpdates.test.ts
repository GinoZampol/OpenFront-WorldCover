 ...
 * transferable `packedPlayerUpdates` tuple buffer (drained from GameImpl),
...
describe("packedPlayerUpdates (GameImpl drain)", () => {
  ...
  test("a stat change is drained as a [smallID, tiles, value, gold, troops] tuple", () => {
    alice.addGold(500n);
    game.executeNextTick();
    const packed = game.drainPackedPlayerUpdates();
    expect(packed).not.toBeNull();
    // Find alice's tuple (other players may have churned too).
    let tuple: number[] | undefined;
    for (let i = 0; i + 4 < packed!.length; i += 5) {
      if (packed![i] === alice.smallID()) {
        tuple = Array.from(packed!.subarray(i, i + 5));
      }
    }
...
describe("GameRunner payload cadence", () => {
  ...
  test("stat churn arrives as packedPlayerUpdates tuples on the view data", () => {
    tick(); // 1
    tick(); // 2
    game.endSpawnPhase();
