...
import { GameMap, normalizeLandValue, TileRef } from "../../core/game/GameMap";
import {
  GameUpdateType,
  GameUpdateViewData,
  SpawnPhaseEndUpdate,
...
import {
  WORLD_COVER_CLASS_INDEX_BY_MAGNITUDE,
  WORLD_COVER_CLASSES,
...
export class GameView implements GameMap {
  ...
  /** Weighted value contributed by each WorldCover class, keyed by smallID. */
  private _worldCoverLandValueBreakdowns = new Map<number, Float64Array>();
  /**
  ...
  public update(gu: GameUpdateViewData) {
    ...
    // Packed per-player stats: [smallID, tilesOwned, landValue, gold, troops]
    ...
    if (packedStats !== undefined) {
      for (let i = 0; i + 4 < packedStats.length; i += 5) {
        ...
        state.landValue = packedStats[i + 2];
        state.gold = packedStats[i + 3];
  ...
  totalLandValue(): number {
    return this._map.totalLandValue();
  }
  ...
  landValue(ref: TileRef): number {
    return this._map.landValue(ref);
  }

  worldCoverLandValueBreakdown(player: PlayerView): Float64Array {
    return (
      this._worldCoverLandValueBreakdowns.get(player.smallID()) ??
      new Float64Array(WORLD_COVER_CLASSES.length)
    );
  }
  ...
  updateTile(tile: TileRef, state: number): boolean {
    if (this._config.gameConfig().gameMap !== GameMapType.WorldCover) {
      return this._map.updateTile(tile, state);
    }

    const previousOwner = this._map.ownerID(tile);
    const previousClassIndex =
      WORLD_COVER_CLASS_INDEX_BY_MAGNITUDE[this._map.magnitude(tile)];
    const previousValue = this._map.landValue(tile);
    const terrainChanged = this._map.updateTile(tile, state);
    const nextOwner = this._map.ownerID(tile);
    const nextClassIndex =
      WORLD_COVER_CLASS_INDEX_BY_MAGNITUDE[this._map.magnitude(tile)];
    const nextValue = this._map.landValue(tile);

    if (previousOwner !== 0 && previousClassIndex !== undefined) {
      this.adjustWorldCoverBreakdown(
        previousOwner,
        previousClassIndex,
        -previousValue,
      );
    }
    if (nextOwner !== 0 && nextClassIndex !== undefined) {
      this.adjustWorldCoverBreakdown(nextOwner, nextClassIndex, nextValue);
    }

    return terrainChanged;
  }

  private adjustWorldCoverBreakdown(
    smallID: number,
    classIndex: number,
    delta: number,
  ): void {
    let breakdown = this._worldCoverLandValueBreakdowns.get(smallID);
    if (breakdown === undefined) {
      breakdown = new Float64Array(WORLD_COVER_CLASSES.length);
      this._worldCoverLandValueBreakdowns.set(smallID, breakdown);
    }
    breakdown[classIndex] = normalizeLandValue(
      Math.max(0, breakdown[classIndex] + delta),
    );
  }
