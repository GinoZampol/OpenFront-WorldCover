...
const DEFAULT_OPTIONS = {
  ...
  compactMap: false,
  worldCoverMapSize: GameMapSize.Normal,
  maxTimer: false,
...
export class SinglePlayerModal extends BaseModal {
  ...
  @state() private worldCoverMapSize: GameMapSize =
    DEFAULT_OPTIONS.worldCoverMapSize;
  ...
  private hasOptionsChanged(): boolean {
    return (
      ...
      (this.selectedMap === GameMapType.WorldCover &&
        this.worldCoverMapSize !== DEFAULT_OPTIONS.worldCoverMapSize) ||
  ...
  private setWorldCoverMapSize = (nextSize: GameMapSize) => {
    if (
      ...
      nextSize !== GameMapSize.Large &&
      nextSize !== GameMapSize.Maximum
    ) {
      return;
    }

    const wasCompact = this.worldCoverMapSize === GameMapSize.Compact;
    const isCompact = nextSize === GameMapSize.Compact;
    this.worldCoverMapSize = nextSize;
    if (wasCompact !== isCompact) {
      this.bots = getBotsForCompactMap(this.bots, isCompact);
      this.nations = getNationsForCompactMap(
        this.nations,
        this.defaultNationCount,
        isCompact,
