...
export { WORLD_COVER_TERRAIN_COLORS } from "../../../../core/game/WorldCover";
...
export interface TerrainColorOverrides {
  ...
  /** Opt-in palette for maps whose terrain magnitudes encode land-cover classes. */
  profile?: "classic" | "worldcover";
}
...
export function encodeTerrainTile(
  ...
  if (isLand && magnitude === 31) {
    ...
    isLand &&
    colors?.profile === "worldcover" &&
    WORLD_COVER_TERRAIN_COLORS[magnitude] !== undefined
  ) {
    // Preserve economic class colors right up to the coast. Ordinary maps
    // use sand there, but that would hide valuable cities and mangroves.
    [r, g, b] = WORLD_COVER_TERRAIN_COLORS[magnitude];
    ...
    if (colors?.profile === "worldcover") {
      [r, g, b] = WORLD_COVER_COASTAL_WATER_COLOR;
    } else {
      // Shoreline water — blend 70% ocean color and 30% white.
