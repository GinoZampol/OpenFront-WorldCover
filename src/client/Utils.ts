...
/**
 * WorldCover values represent population-like capacity. Displaying them at
 * 1,000× makes 7M of game value read as 7B without changing any mechanics.
 */
export function renderWorldCoverValue(
  value: number | bigint,
  fixedPoints?: number,
): string {
  return renderNumber(Number(value) * 1_000, fixedPoints);
}

export function renderWorldCoverTroops(troops: number): string {
  return renderWorldCoverValue(troops / 10);
}
