...
export class BinaryLoaderGameMapLoader implements GameMapLoader {
  ...
  getMapData(map: GameMapType): MapData {
    ...
    const mapData = {
      ...
      map8kBin: this.createLazyLoader(() =>
        loadBinary(mapAssetUrl("map8k.bin")),
      ),
      map8k4xBin: this.createLazyLoader(() =>
        loadBinary(mapAssetUrl("map8k4x.bin")),
      ),
      map10kBin: this.createLazyLoader(() =>
        loadBinary(mapAssetUrl("map10k.bin")),
      ),
      map10k4xBin: this.createLazyLoader(() =>
        loadBinary(mapAssetUrl("map10k4x.bin")),
      ),
