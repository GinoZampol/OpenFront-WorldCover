...
import { WORLD_COVER_CLASS_BY_MAGNITUDE } from "./WorldCover";

export type TileRef = number;

export type LandValueMode = "uniform" | "worldcover";
...
export class GameMapImpl implements GameMap {
  ...
  private totalLandValue_: number;
  ...
  constructor(
    ...
    private readonly landValueMode: LandValueMode = "uniform",
  ) {
    ...
    for (let tile = 0; tile < terrainData.length; tile++) {
      if (this.isLand(tile) && !this.isImpassable(tile)) {
        totalLandValue += this.landValue(tile);
      }
    }
    this.totalLandValue_ = normalizeLandValue(totalLandValue);
  }
  numTilesWithFallout(): number {
  ...
  landValue(ref: TileRef): number {
    if (!this.isLand(ref) || this.isImpassable(ref)) return 0;
    if (this.landValueMode === "uniform") return LAND_VALUE_SCALE;
    return WORLD_COVER_CLASS_BY_MAGNITUDE[this.magnitude(ref)]?.value ?? 1;
  }

  setWater(ref: TileRef): void {
    if (!this.isLand(ref) || this.isImpassable(ref)) return;
    this.totalLandValue_ = normalizeLandValue(
      this.totalLandValue_ - this.landValue(ref),
    );
