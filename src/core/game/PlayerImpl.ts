...
import { andFN, manhattanDistFN, normalizeLandValue, TileRef } from "./GameMap";
import {
...
import { WORLD_COVER_STARTING_LAND_VALUE } from "./WorldCover";
...
export class PlayerImpl implements Player {
  ...
  constructor(
    ...
    if (mg.config().gameConfig().gameMap === GameMapType.WorldCover) {
      this._landValue = WORLD_COVER_STARTING_LAND_VALUE;
    }
   ...
   *
   * tilesOwned / landValue / gold / troops are excluded from partial updates (they churn
   * for every alive player every tick): when any of them changed, a
   * `[smallID, tilesOwned, landValue, gold, troops]` tuple is pushed to `statsOut`
  ...
  adjustLandValue(delta: number): void {
    this._landValue = normalizeLandValue(this._landValue + delta);
    if (this._landValue < 0) {
