...
export class GameImpl implements Game {
  ...
  /** [smallID, tilesOwned, landValue, gold, troops] tuples — see PlayerImpl.toUpdate. */
  private playerStatsTuples: number[] = [];
  ...
  drainPackedPlayerUpdates(): Float64Array | null {
    const tuples = this.playerStatsTuples;
    if (tuples.length === 0) return null;
    const packed = Float64Array.from(tuples);
    tuples.length = 0;
    return packed;
  }
  ...
  conquer(owner: PlayerImpl, tile: TileRef): void {
    ...
    if (previousOwner.isPlayer()) {
      ...
      previousOwner.adjustLandValue(-this._map.landValue(tile));
    }
    this._map.setOwnerID(tile, owner.smallID());
    owner._tiles.add(tile);
    owner.adjustLandValue(this._map.landValue(tile));
