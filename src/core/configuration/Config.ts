...
export class Config {
  ...
  maxTroops(player: Player | PlayerView): number {
    if (this._gameConfig.gameMap === GameMapType.WorldCover) {
      // OpenFront stores troops in tenths and renderTroops() divides by 10 for
      // display. Convert the economic score into internal troop units so the
      // player-facing maximum is exactly equal to land value.
      return player.landValue() * 10;
    }

    const effectiveTiles = player.numTilesOwned();
    const maxTroops =
      player.type() === PlayerType.Human && this.hasInfiniteTroopsFor(player)
        ? 1_000_000_000
        : 2 * (Math.pow(effectiveTiles, 0.6) * 1000 + 50000) +
          player
            .units(UnitType.City)
  ...
  troopIncreaseRate(player: Player | PlayerView): number {
    ...
    if (max <= 0) return -player.troops();
    if (player.troops() >= max) return max - player.troops();
