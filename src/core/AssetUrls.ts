...
export function buildAssetUrl(
  ...
  const unversionedPath = `/${encodeAssetPath(normalizedPath)}`;
  // A path-only base is used by static hosts mounted below the domain root
  // (for example GitHub Pages at /repository-name/). Absolute CDN bases keep
  // their existing manifest-only behavior so unknown files stay same-origin.
  if (baseUrl.startsWith("/")) {
    return `${baseUrl.replace(/\/+$/, "")}${unversionedPath}`;
  }
